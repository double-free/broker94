...
_MdsSyncUdpSample_OnTimeout(MdsApiChannelGroupT *pChannelGroup) {
    /* UDP会话的超时时间 (秒) */
    static const int32  THE_UDP_SESSION_TIMEOUT_MS = 60;
    ...
    if (__spk_unlikely(THE_UDP_SESSION_TIMEOUT_MS > 0
            && currentTime - lastRecvTime > THE_UDP_SESSION_TIMEOUT_MS * 2)) {
        SLOG_ERROR("长时间未接收到任何组播消息, 将主动断开组播通道! " \
                "heartBtInt[%d], lastRecvTime[%" __SPK_FMT_LL__ "d], " \
                "currentTime[%" __SPK_FMT_LL__ "d]",
                THE_UDP_SESSION_TIMEOUT_MS, lastRecvTime, currentTime);
        return ETIMEDOUT;
    }
